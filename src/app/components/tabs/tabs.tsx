...
function CategoryFromURL({ categories, setActiveTab }: {
  categories: string[],
...
const Tabs = () => {
  ...
  const filteredItems =
    activeTab === "all"
      ? products
      : products.filter((item) => item.category.toLowerCase() === activeTab.toLowerCase());
  ...
  if (loading) {
    return (
      <>
        ...
        <Box sx={{ mt: 4, maxWidth: "1200px", mx: "auto", px: { xs: 2, sm: 1 } }}>
      ...
      <Box id="product-tabs" sx={{
        ...
        {categories.map((category) => (
          <Button
            ...
            sx={{
              color: "black", textTransform: "none", border: "none",
      ...
      <Box sx={{ mt: 4, maxWidth: "1200px", mx: "auto", px: 1 }}>
        <Grid container spacing={3} justifyContent="center">
          {filteredItems.map((item) => (
            <Grid key={item._id} size={{ xs: 6, sm: 6, md: 3 }} display="flex" justifyContent="center">
              <Card sx={{
                width: { xs: "100%", sm: 260 }, height: 350,
                ...
                <Link href={`/products/${item._id}`} style={{ textDecoration: "none" }}>
                  <CardMedia
                    component="img" height="180" loading="lazy"
                ...
                <CardContent sx={{ marginTop: '0px', marginBottom: '0px' }}>
                  <Typography variant="h6" fontWeight={600} sx={{
                    ...
                    fontSize: { xs: 13, sm: 15, md: 16 }, lineHeight: 1.3,
                  }}>
                    {item.name}
                  </Typography>
                  ...
                  <Button
                    ...
                    sx={{
                      mt: 2, width: "100%", borderRadius: 16, textTransform: "none",
                      backgroundColor: item.stock === 0 ? "#333" : "#1976d2",
                      marginY: '10px',
                      "&:hover": { backgroundColor: item.stock === 0 ? "#3339" : "#1565c0" },
                      "&.Mui-disabled": { backgroundColor: "#ef4444", color: "#fff" },
                    }}
                    onClick={() => {
                      if (item.stock !== 0) {
                        addToCart({
                          id: item._id, name: item.name,
                          price: item.price, image: item.images?.[0], quantity: 1,
                        });
